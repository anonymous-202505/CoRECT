...
def _load_beir_data(
    ...
    """
    split = "test" if dataset_sub_corpus not in ["MSMARCO"] else "dev"
    ...
    queries = {q["_id"]: q["text"] for q in dataset_queries["queries"]}
...
def load_data(dataset_name: str, dataset_sub_corpus: str):
    ...
    if dataset_name == CoRE_NAME:
        ...
        return _load_beir_data(dataset_sub_corpus)
    else:
